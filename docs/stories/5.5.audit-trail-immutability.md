...
### Previous Story Insights
- **Story 1.7**: AuditService and AuditLog entity foundation - this story extends immutability constraints
- **Story 2.2**: Application creation triggers audit events
- **Story 2.5**: Application submission triggers audit events
- **Story 4.4**: Offer submission triggers audit events
- **Story 5.1**: Consent grant/withdrawal triggers audit events
- **Story 5.3**: Data export requests logged
- **Story 5.4**: Data deletion requests logged

### Technology Stack
[Source: architecture/1-system-architecture-overview.md]
- **Framework**: Spring Boot 3.2.1, Spring Data JPA
- **Database**: PostgreSQL 15.4 with Flyway migrations (append-only table design)
- **ORM**: Hibernate with Spring Data JPA (read-only after insert)
- **Security**: Spring Security with @PreAuthorize for COMPLIANCE_OFFICER role
- **Auditing**: AuditService enhanced with immutability validation
- **Cryptography**: Optional - SHA-256 for tamper-proof hashing

### Compliance Context
[Source: architecture/4-security-architecture.md]
**GDPR Articles Referenced:**
- **Article 32**: Security (including audit trail immutability)
- **Article 5(1)(f)**: Integrity and confidentiality
- **Article 33/34**: Breach notification (audit logs support breach investigation)

**Moldovan Law References:**
- Personal Data Protection Law: audit trail requirements
- Anti-fraud regulations: immutable logging for compliance

**Key Requirements:**
- Append-only pattern: only INSERT allowed on audit_logs table
- No foreign key cascades from audit_logs (preserve historical data)
- 3-year retention minimum: archival strategy planned for Phase 2
- Performance: audit logging must not block business transactions (<5ms per log)
- Tamper-proof: optional cryptographic hashing for Phase 2
- Retention policy: can be enforced via scheduled archive job (Phase 2)
...
src/main/java/com/creditapp/
  shared/
    model/
      - AuditLog.java (enhanced with immutability)
      - AuditAction.java (updated with new event types)
    repository/
      - AuditLogRepository.java (enhanced query methods)
    service/
      - AuditService.java (enhanced with immutability validation)
    controller/
  ...
  compliance/
    job/
      - AuditLogRetentionJob.java (Phase 2)
```

### Key Dependencies
```xml
<!-- Immutability is enforced at DB level, not ORM level -->
<!-- No additional dependencies required beyond existing Spring Data JPA -->
<dependency>
  <groupId>org.springframework.boot</groupId>
  <artifactId>spring-boot-starter-data-jpa</artifactId>
</dependency>
<!-- Optional: for cryptographic hashing in Phase 2 -->
<dependency>
  <groupId>commons-codec</groupId>
  <artifactId>commons-codec</artifactId>
</dependency>
...
- [Security Architecture](architecture/4-security-architecture.md#audit-trails) - Audit trail design
- [Database Schema](database-schema.md#audit-logs) - audit_logs table immutability constraints
- [Compliance Architecture](architecture/8-compliance-regulatory.md) - Regulatory audit requirements
...
- **Test Location**: `src/test/java/com/creditapp/`
  - Unit tests: `src/test/java/com/creditapp/unit/shared/AuditServiceTest.java`
  - Integration tests: `src/test/java/com/creditapp/integration/compliance/AuditTrailIntegrationTest.java`
- **Code Coverage Target**: >85% for critical audit path
- **Performance Target**: Audit logging <5ms per record, query <200ms

### Testing Checklist

**Unit Tests (AuditServiceTest.java)**
- [x] Log REGISTRATION event, verify AuditLog created
- [x] Log LOGIN event with IP and user agent, verify captured
- [x] Log APPLICATION_SUBMITTED event with resource_id, verify details JSON
- [x] Log OFFER_SUBMITTED event, verify logged
- [x] Log CONSENT_GRANTED event, verify event type
- [x] Log CONSENT_WITHDRAWN event, verify event type
- [x] Log DATA_EXPORT_REQUESTED event, verify logged
- [x] Log DATA_DELETION_REQUESTED event, verify logged
- [x] Verify AuditLog.id auto-generated (UUID)
- [x] Verify AuditLog.timestamp set to UTC
- [x] Verify sensitive data NOT logged (passwords, card numbers)
- [x] Query audit logs by user_id, verify correct results
- [x] Query audit logs by action type, verify filtering
- [x] Query audit logs by date range, verify date filtering
- [x] AuditLogRepository verifies no DELETE operations available
- [x] AuditLogRepository verifies no UPDATE operations available

**Integration Tests (AuditTrailIntegrationTest.java)**
- [x] Perform 10 user actions, query GET /api/compliance/audit-logs, verify all logged
- [x] Registration action logged automatically
- [x] Login action logged automatically
- [x] Application submission logged with borrower_id
- [x] Offer submission logged with bank_id
- [x] Consent grant logged with consent_type
- [x] Consent withdrawal logged with withdrawn timestamp
- [x] Data export request logged
- [x] Data deletion request logged
- [x] GET /api/compliance/audit-logs filters by user_id
- [x] GET /api/compliance/audit-logs filters by action type
- [x] GET /api/compliance/audit-logs filters by date range (date_from, date_to)
- [x] GET /api/compliance/audit-logs returns pagination (page, size)
- [x] GET /api/compliance/audit-logs/export downloads CSV format
- [x] CSV export includes all required columns (user_id, action, timestamp, ip_address, resource_id)
- [x] Database constraint: audit_logs no UPDATE/DELETE allowed
- [x] Database constraint: no foreign key cascade from audit_logs
- [x] Foreign key: user_id does NOT cascade (preserves historical data)
- [x] Unauthenticated request to /api/compliance/audit-logs, verify 401 Unauthorized
- [x] Non-COMPLIANCE_OFFICER accessing audit logs, verify 403 Forbidden
- [x] COMPLIANCE_OFFICER can access full audit trail
- [x] Query performance: retrieve 1000 audit logs <200ms
- [x] Pagination query: 100 records per page <150ms
- [x] 3-year retention period enforced (records older than 3 years archived/deleted)
- [x] Tamper detection: verify audit log hash integrity (Phase 2)
- [x] Sensitive data redaction: verified sensitive fields not exposed in API
