...
- [ ] Task 1: Create Scenario Calculator Request/Response DTOs (AC: 1, 4, 5)
  - [ ] Create CalculateScenarioRequest (src/main/java/com/creditapp/borrower/dto/CalculateScenarioRequest.java)
  ...
  - [ ] Create CalculateScenarioResponse (src/main/java/com/creditapp/borrower/dto/CalculateScenarioResponse.java)
  ...
  - [ ] Unit tests: verify validation, required fields, decimal precision

- [ ] Task 2: Create Scenario Calculator Service (AC: 2, 3, 5, 9)
  - [ ] Create ScenarioCalculatorService (src/main/java/com/creditapp/borrower/service/ScenarioCalculatorService.java)
  ...
  - [ ] Unit tests: mock rate card repository, test calculations, test validation

- [ ] Task 3: Create REST Controller Endpoint (AC: 1, 4, 6, 7)
  - [ ] Create or update ScenarioCalculatorController (src/main/java/com/creditapp/borrower/controller/ScenarioCalculatorController.java)
  ...
  - [ ] Add Caching (optional performance boost, AC: 8)
...
- [ ] Task 4: Implement Rate Limiting (AC: 7)
  - [ ] Add Resilience4j dependency to pom.xml
    - artifact: io.github.resilience4j:resilience4j-ratelimiter
  - [ ] Create RateLimiterConfig (src/main/java/com/creditapp/shared/config/RateLimiterConfig.java)
    - @Configuration class
    - @Bean RateLimiterRegistry
    - @Bean RateLimiter for scenario-calculator
      - Limit: 100 requests per minute per IP
      - Timeout: 1 second
      - Exception: RequestNotPermitted
  - [ ] Add @RateLimiter annotation to controller method
    - @RateLimiter(name = "scenario-calculator", fallbackMethod = "rateLimitExceeded")
    - Fallback method: returns 429 Too Many Requests with descriptive error
  - [ ] Custom rate limiter key function: extract IP from request
    - Use CompositeRegistryEventConsumer for per-IP tracking
    - OR use web filter to apply rate limiting
  - [ ] Unit tests: verify rate limit enforced, correct error response

- [ ] Task 5: Create Rate Card Lookup Service (AC: 2, 3)
  - [ ] Create RateCardLookupService (src/main/java/com/creditapp/shared/service/RateCardLookupService.java)
  ...
  - [ ] Error handling: InvalidBankException if not found
  - [ ] Unit tests: verify lookup, caching, error handling

- [ ] Task 6: Add Calculation Utilities (AC: 2, 9)
  - [ ] Create CalculationUtils class (src/main/java/com/creditapp/shared/util/CalculationUtils.java) or reuse from Story 3.3
...
- [ ] Task 7: Create Default Rate Card Configuration (AC: 3)
