...
- [ ] Task 1: Create Database Migrations for Data Export Tracking (AC: 5, 6, 8)
  - [ ] Create V13__Create_Data_Export_Table.sql migration
...
- [ ] Task 2: Create Data Export JPA Entity (AC: 5, 6, 7)
  - [ ] Create DataExport entity (src/main/java/com/creditapp/shared/model/DataExport.java)
    - Fields: id (UUID), borrowerId (UUID), status (ExportStatus enum), format (ExportFormat enum), fileUrl (String nullable), downloadToken (String unique nullable), downloadTokenExpiresAt (LocalDateTime nullable), requestedAt (LocalDateTime), completedAt (LocalDateTime nullable), createdByIp (String), expiresAt (LocalDateTime)
    - @Entity, @Table(name="data_exports")
    - @Enumerated for status and format
  - [ ] Create ExportStatus enum: PENDING, COMPLETED, FAILED
  - [ ] Create ExportFormat enum: JSON, PDF

- [ ] Task 3: Create Data Export Repository (AC: 1)
  - [ ] Create DataExportRepository (src/main/java/com/creditapp/shared/repository/DataExportRepository.java)
    - Extends JpaRepository<DataExport, UUID>
    - Method: findByDownloadToken(String token) returns Optional<DataExport>
    - Method: findPendingByBorrowerId(UUID borrowerId, Pageable pageable) returns List<DataExport>

- [ ] Task 4: Create Data Export Service (AC: 1, 2, 3, 5, 6, 8, 9)
  - [ ] Create DataExportService
    - Method: initiateExport(UUID borrowerId, ExportFormat format, String ipAddress) returns DataExportResponse
      - Create DataExport entity with status=PENDING
      - Generate download token (32-char random string)
      - Queue async job to generate export file
      - Log DATA_EXPORT_REQUESTED audit event
      - Return response with message "Export requested. Download link sent to email."
    - Method: generateExportFile(UUID exportId) (async job)
      - Fetch all borrower data:
        - User profile (name, email, phone, address)
        - All Applications (list with details)
        - All Offers (list with details)
        - All Consents (with timestamps)
        - All audit log entries for borrower
      - Generate JSON file with structured data
      - Optionally generate PDF (human-readable format)
      - Upload to secure S3 bucket
      - Update DataExport: status=COMPLETED, fileUrl=s3_url, downloadTokenExpiresAt=now+24hrs
      - Queue email to borrower with download link
      - Log DATA_EXPORT_COMPLETED audit event
    - Method: downloadExport(String downloadToken, UUID borrowerId) returns FileContent
      - Verify token valid and not expired
      - Verify token belongs to borrower
      - Mark token as used (download_token = NULL)
      - Log DATA_EXPORT_DOWNLOADED audit event
      - Return file content for download
  - [ ] Unit tests: mock repositories, test export initiation and token generation

- [ ] Task 5: Create Data Export REST Controller (AC: 1, 6, 7)
  - [ ] Create BorrowerDataExportController (src/main/java/com/creditapp/borrower/controller/BorrowerDataExportController.java)
    - GET /api/borrower/data-export
      - @PreAuthorize("hasAuthority('BORROWER')")
      - Query parameter: format (JSON, PDF, BOTH; default JSON)
      - Extract borrowerId and ipAddress from SecurityContext
      - Call DataExportService.initiateExport()
      - Return 202 Accepted with message
    - GET /api/borrower/data-export/download
      - Query parameter: token (download token)
      - Call DataExportService.downloadExport(token, borrowerId)
      - Return file for download (Content-Type: application/json or application/pdf)
      - Error handling: 401 Unauthorized, 403 Forbidden (wrong borrower), 404 Not Found, 410 Gone (expired)
  - [ ] Unit tests: mock service, test endpoints
  - [ ] Integration tests: real HTTP requests

- [ ] Task 6: Create Async Job for Export Generation (AC: 5, 9)
  - [ ] Create DataExportJob or use Spring Async
    - Use @Async annotation on generateExportFile method
    - Or create Scheduled task that picks up PENDING exports
    - Monitor for timeouts; timeout after 5 minutes with status=FAILED
  - [ ] Thread pool configuration: max 5 concurrent exports (configurable)
  - [ ] Job monitoring: log start, progress (% of data exported), completion, errors
