...
## Status
In Progress - Phase 1 Complete (Tasks 1-8 Done, Tasks 9-10 TODO)
...
- [x] Task 7: Create Export File Generator (AC: 2, 3, 4, 10)
  - [x] Created ExportFileGenerator service
    - Method: generateJsonExport(UUID borrowerId) returns JsonNode
      - Fetches all borrower data from 6 repositories (User, Application, ApplicationHistory, Offer, GDPRConsent, AuditLog)
      - Structures as JSON with 5 sections:
        - profile: user details (email, name, phone, role, active, createdAt)
        - applications: all applications with pagination (max 1000) + nested history records
        - offers: all offers for all borrower applications
        - consents: GDPR consent history ordered by date
        - auditLog: last 500 audit log entries for borrower
      - Includes all PII as required
    - Method: generatePdfExport(UUID borrowerId) returns byte[]
      - Placeholder for future implementation (iText integration)
  - [x] Injected into DataExportService
  - [x] Integrated into async generateExportFileAsync() method
  - [x] Compilation SUCCESS verified
  - [x] Build SUCCESS verified

- [x] Task 8: Create Email Notification for Export (AC: 5, 6)
  - [x] Created email template: data-export-ready.html
    - Professional HTML design with responsive layout
    - Subject: "Your Data Export is Ready"
    - Includes download link with token parameter
    - Shows file information: format, size, generated timestamp
    - Lists all data sections included (profile, applications, offers, consents, audit log)
    - Important notices: link expiry (24 hours), one-time use, sensitive data warning
    - Instructions: how to verify and store data
    - Plain text fallback version
  - [x] Created DataExportEmailService
    - Loads HTML template and replaces placeholders
    - Builds download link with download token
    - Gets borrower email from UserRepository
    - Formats timestamps and file information
    - Creates plain text version
    - Error handling: doesn't fail export if email send fails
  - [x] Integrated into DataExportService.generateExportFileAsync()
    - Automatically sends email when export completes
    - Uses sendExportReadyEmail() helper method
