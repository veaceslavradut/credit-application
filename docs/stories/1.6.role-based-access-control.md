...
- [ ] Task 2: Configure Method Security (AC: 1, 5)
  ...
  - [ ] Update JwtAuthenticationFilter from Story 1.5
    - Extract role from JWT claims
    - Create GrantedAuthority list with user role
    - Set authorities in SecurityContext Authentication object
  - [ ] Unit tests: verify JWT filter extracts role correctly

- [ ] Task 3: Apply Authorization to User/Auth Endpoints (AC: 1, 2, 3)
  - [ ] Add @PreAuthorize annotations to AuthController methods
  ...
  - [ ] Add @PreAuthorize annotations to UserProfileController (placeholder for Story 1.8)
...
- [ ] Task 4: Create Borrower-Only Endpoints Protection (AC: 1, 2, 3)
  - [ ] Create BorrowerApplicationController (placeholder for Epic 2)
    - GET /api/applications: @PreAuthorize(\"hasAuthority('BORROWER')\")
    - POST /api/applications: @PreAuthorize(\"hasAuthority('BORROWER')\")
    - GET /api/applications/{id}: @PreAuthorize(\"hasAuthority('BORROWER')\")
    - PUT /api/applications/{id}: @PreAuthorize(\"hasAuthority('BORROWER')\")
  - [ ] Document borrower-only endpoints in API_ENDPOINTS.md

- [ ] Task 5: Create Bank Admin-Only Endpoints Protection (AC: 1, 2, 3, 7)
  - [ ] Create BankAdminController (placeholder for Epic 4)
  ...
  - [ ] Document bank admin-only endpoints in API_ENDPOINTS.md

- [ ] Task 6: Create Compliance Officer Endpoints Protection (AC: 1, 2, 4)
  - [ ] Create ComplianceController (placeholder for Epic 5)
  ...
  - [ ] Document compliance officer endpoints in API_ENDPOINTS.md

- [ ] Task 7: Create Access Denied Handler (AC: 3)
  - [ ] Create CustomAccessDeniedHandler (src/main/java/com/creditapp/shared/security/CustomAccessDeniedHandler.java)
  ...
  - [ ] Register CustomAccessDeniedHandler in SecurityConfig
    - http.exceptionHandling().accessDeniedHandler(customAccessDeniedHandler)
  - [ ] Unit tests: verify 403 response format

- [ ] Task 8: Create Authorization Utility Service (AC: 5)
  - [ ] Create AuthorizationService (src/main/java/com/creditapp/shared/security/AuthorizationService.java)
  ...
  - [ ] Unit tests: verify role checking methods

- [ ] Task 9: Add Custom Authorization Annotations (AC: 1, 4)
  - [ ] Create @RequiresBorrowerRole annotation (src/main/java/com/creditapp/shared/security/RequiresBorrowerRole.java)
    - Meta-annotation: @PreAuthorize(\"hasAuthority('BORROWER')\")
    - Used for cleaner controller code
  - [ ] Create @RequiresBankAdmin annotation
    - Meta-annotation: @PreAuthorize(\"hasAuthority('BANK_ADMIN')\")
  - [ ] Create @RequiresComplianceOfficer annotation
    - Meta-annotation: @PreAuthorize(\"hasAuthority('COMPLIANCE_OFFICER')\")
  - [ ] Create @RequiresAnyRole(roles = {\"BANK_ADMIN\", \"COMPLIANCE_OFFICER\"}) annotation
    - Meta-annotation: @PreAuthorize(\"hasAnyAuthority(...)\")
  - [ ] Update controllers to use custom annotations (optional, for cleaner code)

- [ ] Task 10: Unit Tests for Authorization Logic (AC: 6)
  - [ ] Create AuthorizationUnitTest (src/test/java/com/creditapp/unit/security/AuthorizationUnitTest.java)
  ...
  - [ ] Mock SecurityContext, Authentication objects
