...
- [ ] Task 1: Create Offer Submission DTOs
  - [ ] Create OfferSubmissionRequest (applicationId, acceptCalculatedOffer, overrideAPR, overrideFees, overrideProcessingTime)
  - [ ] Create OfferSubmissionResponse (offerId, APR, fees, processingTime, monthlyPayment, submittedAt)
  - [ ] Create OfferCalculationDTO for display

- [ ] Task 2: Create Offer Submission Service
  - [ ] Create BankOfferSubmissionService
...
- [ ] Task 3: Create Offer Submission Controller
  - [ ] Create BankOfferSubmissionController
...
- [ ] Task 4: Monthly Payment Calculation (AC: 5, 6)
  - [ ] CRITICAL: Use calculation formula from DECISION_2_CALCULATION_FORMULA.md
    - Formula: M = P × [r(1 + r)^n] / [(1 + r)^n - 1]
    - P = FULL loan amount (fees are NOT deducted from principal)
    - r = monthly rate (APR / 100 / 12)
    - n = term in months
  - [ ] Reference Decision 2 test vectors for validation
  - [ ] Use BigDecimal with ROUND_HALF_UP, scale=2 for amounts, scale=4 for APR
  - [ ] Total Cost = (monthlyPayment × months) + fees (additive, not deductive)
  - [ ] Recalculate when APR changes
  - [ ] Include in response display (AC6)

- [ ] Task 5: Idempotency Implementation (AC: 7)
  - [ ] Check: SELECT * FROM offer WHERE bank_id = ? AND application_id = ?
    - If exists: return 200 OK with existing offer (NOT 201, same offer ID)
    - If not exists: create new offer, return 201 CREATED
  - [ ] Idempotency means: same request = same offer (not new offer with same values)
  - [ ] Store offer with unique constraint on (bank_id, application_id) to prevent duplicates

- [ ] Task 6: Email Notification
  - [ ] Queue email to borrower (use Story 1.9 email service)
...
- [ ] Task 7: Audit Logging
  - [ ] Create audit log entry: OFFER_SUBMITTED_BY_BANK
    - Fields: bankId, applicationId, offerId, APR, fees, processingTime, timestamp

- [ ] Task 8: Unit Tests
  - [ ] Test accept calculated offer (AS-IS)
  - [ ] Test override APR with recalculation
  - [ ] Test APR validation (0.5-50% range)
  - [ ] Test monthly payment calculation accuracy
  - [ ] Test idempotency (duplicate submission)
  - [ ] Test email queue

- [ ] Task 9: Integration Tests
  - [ ] Create application, submit offer with AS-IS
  - [ ] Verify offer saved with correct values
  - [ ] Submit offer with APR override, verify recalculation
  - [ ] Test idempotency: submit same offer twice, verify same response
  - [ ] Verify email queued
