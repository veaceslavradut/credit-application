...
- [ ] Task 9: Create Unit Tests (AC: 1, 2, 3, 5) - TODO: Create comprehensive tests
...
### Completion Notes
**Tasks 1-8 COMPLETE (PARTIAL - Core Infrastructure)**
- Task 1: OfferDocumentUploadRequest, OfferDocumentMetadata, OfferDocumentsListResponse DTOs created ✓
- Task 2: OfferDocument entity with @Index on (offerId, documentType), OfferDocumentRepository with custom queries ✓
- Task 3: OfferDocumentUploadService with file validation, S3 upload, audit logging ✓
- Task 4: OfferDocumentRetrievalService with access control and pre-signed URL regeneration ✓
- Task 5: OfferDocumentDownloadService with virus scan status checking and audit logging ✓
- Task 6: OfferDocumentController with 3 endpoints (POST upload, GET list, GET download) ✓
- Task 7: S3DocumentStorageService with uploadFile, generatePresignedUrl, deleteFile; S3Configuration bean; AWS SDK v2 dependency added ✓
- Task 8: FileValidationService for file size (10MB), MIME type, and extension validation ✓

**Key Implementation Details:**
- File validation: PDF, DOC, DOCX, XLS, XLSX (max 10MB)
- S3 Storage: AES256 encryption, private ACL, pre-signed URLs (24-hour expiration)
- S3 Key format: "offers/{offerId}/{documentType}_{timestamp}_{fileName}"
- Audit events: DOCUMENT_UPLOADED (with offerId, documentId, type, size, officer)
- Audit events: DOCUMENT_DOWNLOADED (with offerId, documentId, userId, type)
- Access control: Borrower can view/download own offer documents, officer can upload
...
4. OfferDocument.java (69 lines) - JPA entity with indexes
...
- pom.xml: Added AWS S3 SDK v2 dependency (software.amazon.awssdk:s3:2.24.0)
- AuditAction.java: Added DOCUMENT_DOWNLOADED enum value

**Remaining Tasks (9-15):**
- Task 9-11: Unit, Controller, Integration tests (TODO)
- Task 12: API documentation update
- Task 13-15: Admin features (optional)

**Known Limitations:**
- Virus scanning marked as CLEAN by default (MVP - ClamAV integration skipped)
- getUserId() in controller returns placeholder UUID (needs RequestContextService integration)
- Officer name lookup in retrieval service marked TODO
- Bank association validation for officers is simplified

### File List
1. OfferDocumentUploadRequest.java (15 lines, bank/dto)
2. OfferDocumentMetadata.java (22 lines, bank/dto)
3. OfferDocumentsListResponse.java (17 lines, bank/dto)
4. OfferDocument.java (69 lines, bank/model)
5. OfferDocumentRepository.java (27 lines, bank/repository)
6. FileValidationService.java (72 lines, shared/service)
7. S3Configuration.java (54 lines, shared/config)
8. S3DocumentStorageService.java (157 lines, shared/service)
9. OfferDocumentUploadService.java (123 lines, bank/service)
10. OfferDocumentRetrievalService.java (96 lines, bank/service)
11. OfferDocumentDownloadService.java (82 lines, bank/service)
12. OfferDocumentController.java (118 lines, bank/controller)
13. pom.xml (modified - AWS S3 SDK dependency)
14. AuditAction.java (modified - added DOCUMENT_DOWNLOADED)
