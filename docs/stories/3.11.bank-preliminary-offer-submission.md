...
- [ ] Task 1: Create Offer Submission Request/Response DTOs (AC: 1, 2)
  - [ ] Create BankOfferSubmissionRequest (src/main/java/com/creditapp/bank/dto/BankOfferSubmissionRequest.java)
  ...
  - [ ] Create BankOfferSubmissionResponse
...
- [ ] Task 2: Create Offer Submission Service (AC: 3, 4, 5, 6)
  - [ ] Create BankOfferSubmissionService (src/main/java/com/creditapp/bank/service/BankOfferSubmissionService.java)
...
- [ ] Task 3: Create Offer Submission Controller (AC: 1, 9)
  - [ ] Create BankOfferSubmissionController (src/main/java/com/creditapp/bank/controller/BankOfferSubmissionController.java)
  ...
  - [ ] Unit tests: mock service, test request validation

- [ ] Task 4: Implement APR Validation (AC: 3)
  - [ ] Create APRValidator class (src/main/java/com/creditapp/shared/validator/APRValidator.java)
...
- [ ] Task 6: Create Offer Status Differentiation (AC: 5)
  - [ ] Ensure OfferStatus enum distinguishes:
  ...
  - [ ] Use case: Distinguish system vs. bank-submitted offers in UI

- [ ] Task 7: Create Borrower Notification (AC: 7)
  - [ ] Create BankOfferSubmissionEmailService (src/main/java/com/creditapp/shared/service/BankOfferSubmissionEmailService.java)
  ...
  - [ ] Unit tests: mock email service, verify content

- [ ] Task 8: Create Audit Logging (AC: 8)
  - [ ] Add BankOfferSubmissionService logging:
  ...
  - [ ] Also log if submission overrides system offer:
    - Event: OFFER_OVERRIDE
    - Fields: original offerId, new offerId, reason

- [ ] Task 9: Create Unit Tests (AC: 2, 3, 4)
  - [ ] Create BankOfferSubmissionServiceTest (src/test/java/com/creditapp/unit/bank/BankOfferSubmissionServiceTest.java)
    - Test 1: Submit valid offer - created successfully
    - Test 2: APR too low (< 4%) - validation error
    - Test 3: APR too high (> 20%) - validation error
    - Test 4: Loan amount mismatch - validation error
    - Test 5: System calculates monthly payment - calculated correctly
    - Test 6: System calculates total cost - calculated correctly
    - Test 7: Custom monthly payment provided - override used
    - Test 8: Custom total cost provided - override used
    - Test 9: Offer status = SUBMITTED - correct status
    - Test 10: Email sent - borrower notified
    - Test 11: Audit event logged - audit trail recorded
    - Test 12: Override system offer - marks old as withdrawn, creates new
    - Test 13: Unauthorized officer - 403 error
    - Test 14: Application not found - 404 error
  - [ ] Mock: ApplicationRepository, OfferRepository, OfferSubmissionEmailService, AuditService

- [ ] Task 10: Create Controller Tests (AC: 1)
  - [ ] Create BankOfferSubmissionControllerTest (src/test/java/com/creditapp/unit/bank/BankOfferSubmissionControllerTest.java)
    - Test 1: POST /api/bank/offers/submit returns 201 CREATED
    - Test 2: Valid request body accepted
    - Test 3: Invalid APR - 400 Bad Request
    - Test 4: Unauthenticated - 401 Unauthorized
    - Test 5: Not bank officer - 403 Forbidden
    - Test 6: Application not found - 404 Not Found
    - Test 7: Response includes all fields (offerId, apr, monthlyPayment, etc.)
  - [ ] Mock: BankOfferSubmissionService

- [ ] Task 11: Create Integration Tests (AC: 1, 2, 3, 4, 5, 10)
  - [ ] Create BankOfferSubmissionIntegrationTest (src/test/java/com/creditapp/integration/bank/BankOfferSubmissionIntegrationTest.java)
    - Setup: Create application, bank officer, system-calculated offer
    - Test 1: POST submit offer - created successfully
    - Test 2: Offer status = SUBMITTED - correct status
    - Test 3: Offer fields populated - all required fields set
    - Test 4: APR validation - valid range enforced
    - Test 5: Monthly payment calculated - correct calculation
    - Test 6: Total cost calculated - correct calculation
    - Test 7: Email sent to borrower - notification sent
    - Test 8: Audit event logged - OFFER_SUBMITTED_BY_BANK event
    - Test 9: Override system offer - marks old as withdrawn
    - Test 10: Borrower can select submitted offer (AC9) - verify offer selection works
    - Test 11: Unauthorized access - 403 error
  - [ ] Use: TestRestTemplate, TestContainers (PostgreSQL)

- [ ] Task 12: Create API Documentation (AC: 1, 2)
